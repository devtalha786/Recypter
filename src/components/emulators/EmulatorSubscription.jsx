...
const EmulatorSubscriptionContent = () => {
  ...
  const subscriptionPlans = [
    {
      ...
      type: "For LifeTime Emulator",
      service: "Emulator",
      features: ["Use the emulator forever, no limits!"],
      price: "$20",
      period: "For LifeTime Emulator",
    },
  ];
  const handleCheckout = async (plan) => {
    if (!user?.uid) {
      setIsLoginModalOpen(true); // Open the login modal if user is not logged in
      return;
    }
    try {
            ...
            <div>
              ...
              <ul className="space-y-2 mb-6">
                {plan?.features?.map((feature, idx) => (
                  <li key={idx} className="text-gray-600 font-bold flex items-center">
...
const EmulatorSubscription = () => (
  <Elements stripe={stripePromise}>
    <EmulatorSubscriptionContent />
  </Elements>
);

export default EmulatorSubscription;
